...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        favJobFragment = supportFragmentManager.findFragmentByTag("FavJobFragment") as? FavoriteVacanciesFragment
        ...
        binding.bottomNavigation.setOnItemSelectedListener { menuItem ->
            when (menuItem.itemId) {
                R.id.navigation_search -> {
                    mainViewModel.navigateTo(R.id.navigation_search)
                    showFragment(jobFragment)
                }
                R.id.navigation_favorites -> {
                    mainViewModel.navigateTo(R.id.navigation_favorites)
                    showFragment(favJobFragment)
                }
            }
            true
        }
        ...
        lifecycleScope.launch {
            mainViewModel.currentFragment.collect { fragment ->
                fragment?.let {
                    showFragment(it)
                }
            }
